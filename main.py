...
def hash_flutter(version, path):
    ...
    for folder in flutter_folders:
        ...
        try:
            ...
            start_index = output.find(flutter_file + ':') + len(flutter_file) + 1
...
# corresponding .csv files.
def hash_react_native(version, path):
    path = add_slash_to_path(path)

    for folder in react_native_folders:
        csv_file = f'files/react_native/{folder}.csv'
        try:
            print(multiple_files_command(path + folder + '\\', react_native_file))
            output = subprocess.check_output(multiple_files_command(path + folder + '\\', react_native_file)).decode("utf-8")

            # TODO : not yet edited
            for output_row in output:
                print("output_row", output_row)
                start_filename_index = output_row.find('libreact')
                end_filename_index = output_row.find('.so:') + 3
                filename = output_row[start_filename_index+1: end_filename_index].strip()
                print("filename", filename)

                start_index = output_row.find('.so:') + 4
                end_index = output_row.find('CertUtil', start_index)
                output_hash = output_row[start_index+1: end_index].strip()
...
if __name__ == '__main__':
    ...
    if glob.glob(path + react_native_folders[0] + '\\' + react_native_file):
        hash_react_native(sys.argv[1], sys.argv[2])
